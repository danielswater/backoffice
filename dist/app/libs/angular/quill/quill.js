	    ...
	    value: function buildButtons(buttons, icons) {
	      buttons.forEach(function (button) {
	        ...
	        className.split(/\s+/).forEach(function (name) {
				if(name == 'ql-link' || name == 'ql-video'){ // RETIRAR LINK E VIDEO DA TOOLBAR - Luciano Almeida - 2017-03-01 15:29
					return false;
				} else {
	          ...
	          if (fileInput == null) {
	            ...
	            fileInput.addEventListener('change', function () {
	              if (fileInput.files != null && fileInput.files[0] != null) {
					if(fileInput.files[0].type != 'image/jpg' && fileInput.files[0].type != 'image/jpeg' && fileInput.files[0].type != 'image/png' && fileInput.files[0].type != 'image/JPG' && fileInput.files[0].type != 'image/JPEG' && fileInput.files[0].type != 'image/PNG'){
						alert('Este tipo de arquivo não é uma imagem.');
					} else {
