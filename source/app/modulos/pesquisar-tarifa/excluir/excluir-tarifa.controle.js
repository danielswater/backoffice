define(['angularAMD', 'TarifaServico'], function(app) {
	app.controller('ExcluirTarifaControle', ExcluirTarifaControle);
		ExcluirTarifaControle.$inject = ['$scope', '$rootScope', '$http','$uibModalInstance', 'modal', 'ModalFabrica', 'MensagensFabrica', 'TarifaServico'
	];

			function ExcluirTarifaControle($scope, $rootScope, $http, $uibModalInstance, modal, ModalFabrica, MensagensFabrica, TarifaServico
		) {
		...
		$scope.excluir = function() {
		$scope.espera = TarifaServico.deletar(
			modal.objeto.id,
